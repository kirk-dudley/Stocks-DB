...
--IF OBJECT_ID('Add_Stage_Exchange') IS NOT NULL
--DROP PROCEDURE Add_EOD_Historic_Sales
--GO
...
--Creates a list of files and exchanges, having inline joins was killing performance

--DECLARE	@FE TABLE (F_FK INT, E_FK INT);
	...
	FROM	Files 
			JOIN
			Exchanges
				ON	LEFT(F_Short_Name, CHARINDEX('_', F_Short_Name)-1) = E_Name
	WHERE	CHARINDEX('_', F_Short_Name) > 1
	;
...
--Checks for missing symbols
	...
	FROM	Stage_EOD_Daily 
			...
			Daily_Activity
				...
				AND	S_PK = DA_S_FK
			JOIN
			Files F2				--This is to check for "locked" files in previously loaded data, it DOES NOT join to stage. 
				ON	DA_F_FK = F2.F_PK
	WHERE	F2.F_Is_Locked = 0
	AND		(	DA_Close <> SED_Close
			OR	DA_F_FK <> SED_F_FK
			OR	DA_High <> SED_High
			OR	DA_Low <> SED_Low
			OR	DA_Open <> SED_Open
			OR	DA_OpenInt <> SED_OpenInt
			OR	DA_Volume <> SED_Volume
			)
	;
...
GO--Get_Top_N_By_Exchange 'NASDAQ', 37, 'M', 6, 2
